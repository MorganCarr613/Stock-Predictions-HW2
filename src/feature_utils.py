...
def extract_features():
    ...
    stk_tickers = ['KO', 'PEP', 'KDP']
    ccy_tickers = ['DEXCAUS', 'DEXUSEU']
    idx_tickers = ['SP500', 'NASDAQCOM', 'DGS10']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'KO')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('PEP', 'KDP'))]).diff(return_period)
